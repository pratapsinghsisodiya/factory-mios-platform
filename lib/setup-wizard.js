...
function isPublicPath(p) {
  const publicPaths = [
    '/welcome', '/setup/connectors', '/setup/manage', '/setup/data', '/setup/dashboard',
    '/health', '/login',
...
function addDataSource(plantId, source) {
  ...
  const idx = source.id
    ? plant.data_sources.findIndex(x => x.id === source.id)
    : plant.data_sources.findIndex(x =>
        x.type === entry.type && x.topic === entry.topic && x.brokerUrl === entry.brokerUrl
      );
  if (idx >= 0) plant.data_sources[idx] = { ...plant.data_sources[idx], ...entry, id: plant.data_sources[idx].id };
  else plant.data_sources.push(entry);
  saveState(s);
  return plant.data_sources[idx >= 0 ? idx : plant.data_sources.length - 1];
}

function removePlant(plantId) {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  const idx = s.plants.findIndex(p => p.plant_id === plantId);
  if (idx < 0) throw new Error('Plant not found');
  const removed = s.plants.splice(idx, 1)[0];
  saveState(s);
  return removed;
}

function removeMachine(plantId, machineId) {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  const plant = s.plants.find(p => p.plant_id === plantId);
  if (!plant) throw new Error('Plant not found');
  const idx = (plant.machines || []).findIndex(m => m.machine_id === machineId);
  if (idx < 0) throw new Error('Machine not found');
  const removed = plant.machines.splice(idx, 1)[0];
  saveState(s);
  return removed;
}

function removeDataSource(plantId, sourceId) {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  const plant = s.plants.find(p => p.plant_id === plantId);
  if (!plant) throw new Error('Plant not found');
  const idx = (plant.data_sources || []).findIndex(x => x.id === sourceId);
  if (idx < 0) throw new Error('Data source not found');
  const removed = plant.data_sources.splice(idx, 1)[0];
  saveState(s);
  return removed;
}
...
function removeDashboard(dashboardId) {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  const idx = s.dashboards.findIndex(d => d.id === dashboardId);
  if (idx < 0) throw new Error('Dashboard not found');
  const removed = s.dashboards.splice(idx, 1)[0];
  saveState(s);
  return removed;
}

function removeMetricRelation(relationId) {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  const idx = s.metric_relations.findIndex(r => r.id === relationId);
  if (idx < 0) throw new Error('KPI formula not found');
  const removed = s.metric_relations.splice(idx, 1)[0];
  saveState(s);
  return removed;
}

function dedupeDataSources() {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  let removed = 0;
  for (const plant of s.plants || []) {
    const seen = new Map();
    plant.data_sources = (plant.data_sources || []).filter(ds => {
      const key = `${ds.type}|${ds.topic || ''}|${ds.brokerUrl || ''}|${ds.name || ''}`;
...
function dedupeDashboards() {
  const s = loadState();
  if (s.frozen) throw new Error('Setup is frozen. Unfreeze first.');
  const seen = new Map();
  let removed = 0;
  s.dashboards = (s.dashboards || []).filter(d => {
    const key = `${d.name || ''}|${d.plant_id || ''}|${JSON.stringify(d.widgets || [])}`;
    if (seen.has(key)) { removed++; return false; }
    seen.set(key, d.id);
    return true;
  });
...
function stepCompletion(s) {
  const hasPlantMachine = (s.plants || []).some(p => (p.machines || []).length > 0);
  const hasSource = (s.plants || []).some(p => (p.data_sources || []).length > 0);
  return {
    1: Boolean(s.has_superadmin) && hasPlantMachine,
    2: hasSource,
    3: hasSource,
    4: (s.dashboards || []).length > 0,
    5: (s.metric_relations || []).length > 0 || s.frozen,
  };
}
...
function getPublicStatus() {
  ...
  return {
    ...
    superadmin: s.superadmin ? { username: s.superadmin.username, created_at: s.superadmin.created_at } : null,
    plants: s.plants.map(p => ({
      ...
      data_sources: (p.data_sources || []).map(ds => ({
        id: ds.id, type: ds.type, name: ds.name, status: ds.status, topic: ds.topic,
        brokerUrl: ds.brokerUrl ? String(ds.brokerUrl).replace(/:[^:@/]+@/, ':***@') : undefined,
      })),
    })),
    metric_relations: (s.metric_relations || []).map(r => ({ id: r.id, name: r.name, formula: r.formula, plant_id: r.plant_id, machine_id: r.machine_id })),
    dashboards: (s.dashboards || []).map(d => ({ id: d.id, name: d.name, plant_id: d.plant_id, widgets_count: (d.widgets || []).length })),
    ...
    step_completion: stepCompletion({
      ...
      metric_relations: s.metric_relations,
      frozen: s.frozen,
    }),
